...
import {
  ...
  getProductById,
  resetProductState,
} from '../../services/Products/ProductsActions.jsx'
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await dispatch(getProductById(getProductSlug))
          ...
          <section className='pdTopSection w-[screen] md:flex-row flex-col justify-center items-start   '>
            {(loading || cartLoading || wishlistLoading) && (
              <div className='loader'>
                <ClipLoader
                  color={'#52ab98'}
                  loading={cartLoading || loading || wishlistLoading}
                  size={25}
                  aria-label='Loading Spinner'
                  data-testid='loader'
                />
              ...
              (item) =>
                ...
                item.category == aProduct?.category && (
                  <section className='homeContainer '>
                    <p className='pl-1 text-center md:text-left blogMainHead '>
                      You May Also Like{' '}
                    </p>
                    <Swiper
