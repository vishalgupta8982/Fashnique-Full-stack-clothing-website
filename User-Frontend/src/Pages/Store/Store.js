...
const Store = () => {
  ...
  const updateURL = ({
    ...
    discounts.forEach((discount) =>
      queryParams.append("discount[gte]", discount)
    );
  ...
  return (
    <>
      <div
        className={`flex w-[screen]  ${isColorDropdownOpen ? `min-h-[210vh]` : `min-h-[77vh]`
          } p-2 store`}
      >
                      ...
                      color.map((item) => (
                        ...
                        >
                          <input
                            ...
                            onChange={() =>
                              handleCheckboxChange("color", item.title)
                            }
        ...
        <section className="productSectionStore w-[100%]">
          ...
          <div className="hidden md:flex sortForMdScreen ">
            ...
            <p className="storeSortForMd">Showing {Product?.data?.product.length != 0 ? ((page - 1) * 8 + 1) : 0}-{Product?.data?.product.length} of {Product?.data?.totalPages * 8 < 8 ? Product?.data?.totalPages * 8 : Product?.data?.product.length} results </p>
          </div>
          {loading ? (
          ...
          <div className="nextPage">
            {page > 1 ? (<span onClick={() => handleCheckboxChange("page", parseInt(page) - 1)}>
              <RiArrowDropLeftLine className="pageNumber" /></span>) : (<RiArrowDropLeftLine className="pageNumberEnd" />)}
            {[...Array(Math.min(3, Product.data?.totalPages || 0))].map((_, index) => {
              const pageNumber = page < 2 ? index + 1 : Math.min(Product.data?.totalPages - 2 + index, Math.max(1, page - 1 + index));
              return (
                <p
                  key={index}
            ...
            {page < Product.data?.totalPages ? (<span onClick={() => handleCheckboxChange("page", parseInt(page) + 1)}>
              <RiArrowDropRightLine className="pageNumber" /></span>) : (<RiArrowDropRightLine className="pageNumberEnd" />)}
            </div></div>)}
        </section>
      </div>
    </>
